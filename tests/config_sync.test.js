...
test('resendDict: missing auto_return_start defaults to 1 (ON)', () => {
  assert.strictEqual(resendDict(store({ timer_config: 'a\x1f60' })).AutoReturnStart, 1);
});

test('resendDict: saved auto_return_start "0" round-trips to 0', () => {
  assert.strictEqual(resendDict(store({ timer_config: 'a\x1f60', auto_return_start: '0' })).AutoReturnStart, 0);
});

test('resendDict: missing auto_return_stop defaults to 1 (ON)', () => {
  assert.strictEqual(resendDict(store({ timer_config: 'a\x1f60' })).AutoReturnStop, 1);
});

test('resendDict: saved auto_return_stop "0" round-trips to 0', () => {
  assert.strictEqual(resendDict(store({ timer_config: 'a\x1f60', auto_return_stop: '0' })).AutoReturnStop, 0);
});
